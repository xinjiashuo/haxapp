<template>
  <view class="page" @click="closeActionMenu">
    ...
    <view v-if="!keyword && searchHistory.length" class="search-history">
      <view class="search-history-head"><text>最近搜索</text><text @click="clearSearchHistory">清除</text></view>
      <view class="search-history-tags"><text v-for="item in searchHistory" :key="item" @click="useSearchHistory(item)">{{ item }}</text></view>
    </view>
...
const searchHistoryKey = 'hax_moment_search_history'
...
const loadSearchHistory = () => {
  const value = uni.getStorageSync(searchHistoryKey)
  searchHistory.value = Array.isArray(value) ? value.filter((item) => typeof item === 'string' && item) : []
}
const rememberSearch = () => {
  const value = keyword.value
  if (!value) return
  searchHistory.value = [value, ...searchHistory.value.filter((item) => item !== value)].slice(0, 8)
  uni.setStorageSync(searchHistoryKey, searchHistory.value)
}
const clearSearchHistory = () => {
  searchHistory.value = []
  uni.removeStorageSync(searchHistoryKey)
}
const useSearchHistory = (value) => {
  keyword.value = value
  loadFeed()
}
const searchFeed = () => { rememberSearch(); loadFeed() }
...
onShow(() => { loadSearchHistory(); loadFeed(); loadTags() })
</script>
