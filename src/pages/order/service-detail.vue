...
const canCancel = computed(() => ['pending_quote', 'pending_price_confirmation', 'pending_payment', 'pending', 'confirmed'].includes(order.value?.status))
const rescheduleEligible = computed(() => ['pending_quote', 'pending_price_confirmation', 'pending_payment', 'pending', 'confirmed'].includes(order.value?.status) && Number(order.value?.reschedule_count || 0) < Number(order.value?.reschedule_limit || 2))
const rescheduleCutoffReached = computed(() => {
  if (!order.value?.appointment_time) return false
  const appointment = new Date(String(order.value.appointment_time).replace(' ', 'T')).getTime()
  const cutoffHours = Number(order.value.reschedule_cutoff_hours || 0)
  return cutoffHours > 0 && Number.isFinite(appointment) && appointment <= Date.now() + cutoffHours * 60 * 60 * 1000
})
const canReschedule = computed(() => rescheduleEligible.value && !rescheduleCutoffReached.value)
